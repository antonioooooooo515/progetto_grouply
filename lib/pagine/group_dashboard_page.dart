...
class _GroupDashboardPageState extends State<GroupDashboardPage>
  ...
  Future<void> _deleteSelectedItems() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            loc.t('elements_deleted', params: {'count': count.toString()}),
          ),
        ),
      );
      _cancelSelection();
  ...
  // ----------------------------------------------------------------------
  // ADMIN: elimina tutto il gruppo (dati + chat + doc group)
  // ----------------------------------------------------------------------

  Future<void> _deleteQueryInBatches(Query query, {int batchSize = 300}) async {
    while (true) {
  ...
  Future<void> _deleteGroupMessages({int batchSize = 300}) async {
    ...
    await _deleteQueryInBatches(messagesQuery, batchSize: batchSize);
  }

  Future<void> _deleteEntireGroup() async {
    await _deleteGroupMessages();
  ...
  Future<void> _leaveGroup() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          isAdmin
              ? loc.t('admin_delete_group')
              : loc.t('dialog_leave_content'),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
          ...
          TextButton(
            onPressed: () => Navigator.pop(ctx, true),
            child: Text(
              isAdmin ? loc.t('info_delete_group') : loc.t('info_leave_group'),
              style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold),
            ),
          ),
        ],
    ...
    try {
      if (isAdmin) {
        // ADMIN: elimina tutto
        await _deleteEntireGroup();

        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(loc.t('group_delete_confirm'))),
        );

        Navigator.of(context).pushNamedAndRemoveUntil('/home', (r) => false);
      } else {
        // UTENTE NORMALE: esce dal gruppo
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            loc.t('leave_error', params: {'error': e.toString()}),
          ),
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: _isSelectionMode
        ...
        title: Text(
          loc.t('items_selected', params: {'count': _selectedIds.length.toString()}),
        ),
        actions: [
          IconButton(
                        ...
                        onTap: () {
                          if (isSelectionMode) {
                            ...
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => EventDetailsPage(eventId: eventId, isAdmin: isAdmin),
                              ),
                            );
                          }
                        },
...
class _EventCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (customTitle != null && customTitle.isNotEmpty) {
      ...
      title = "${event['homeTeam'] ?? loc.t('team_generic')} vs ${event['awayTeam'] ?? loc.t('team_generic')}";
    }
        ...
        children: [
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: Icon(_getSportIcon(groupSport), color: colors.primary, size: 28),
                      ),
                      const SizedBox(width: 12),
                      Text(
                        groupSport,
                      ...
                      return InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => UserProfilePage(userId: adminId, userName: name),
                            ),
                          );
                        },
                        child: Row(
                          children: [
                            CircleAvatar(
                              ...
                              child: imageProvider == null ? const Icon(Icons.person, size: 20) : null,
                            ),
                            const SizedBox(width: 12),
                            Text(
                              name,
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: Text(
                isGroupAdmin ? loc.t('info_delete_group') : loc.t('info_leave_group'),
                style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold),
              ),
